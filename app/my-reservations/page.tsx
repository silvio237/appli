...
const Page = () => {
  ...
  const deleteReservation = async (id: string) => {
    try {
      ...
      console.error(error);
      setError("Une erreur s'est produite lors de la suppression de la réservation.");
    }
  };
          ...
          {reservations.length === 0 ? (
            ...
            <ul className="grid md:grid-cols-2 gap-4">
              {reservations.map((reservation) => (
                <li
                  key={reservation.id}
                  className="flex items-center mb-5 border-base-300 border p-5 rounded-2xl w-full h-60"
                >
                  <Image
                    src={reservation.room.imgUrl ? reservation.room.imgUrl : "/placeholder.jpg"}
                    alt={reservation.room.id}
                    width={400}
                    height={400}
                    quality={100}
                  ...
                  <div className="ml-6">
                    <div className="flex flex-col md:flex-row md:items-center">
                      ...
                      <h1 className="font-bold text-xl ml-2">{reservation.room.name}</h1>
                    </div>
                    <div className="my-2">
                      <p className="flex">
