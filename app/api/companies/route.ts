...
export async function POST(request: Request) {
    try {
        ...
        if (!email || !companyName) {
            return NextResponse.json({ error: 'Email et nom de l\'entreprise sont requis.' }, { status: 400 });
        }
        ...
        if (existingCompany) {
            return NextResponse.json({ message: 'Une entreprise avec ce nom existe déjà.' }, { status: 409 });
        }
        ...
        return NextResponse.json({ message: 'Entreprise créée avec succès.', company: newCompany }, { status: 201 });
