        ...
        const reservations = await Promise.all(
            timeSlots.map(async (slot) => {
                ...
                return prisma.reservation.create({
                    data: {
                        userId: user.id,  // Utilisation de user.id ici
                        roomId,
                        reservationDate,
                        startTime,
                        endTime,
...
export async function GET(request: Request) {
    try {
        ...
        // Récupération de l'utilisateur et ses réservations
        const user = await prisma.user.findUnique({
            where: { email },
            include: {
                reservations: {
        ...
        // Exclusion de userId dans la réponse
...
export async function DELETE(request: Request) {
    try {
        ...
        // Suppression de la réservation par ID
